...
# Author: Fernando Colchero & Johanna Staerk

# This code calculates a decision tree and the cost-benefit for conservation
# actions of European species (eudat red list Europe)
...
eudat    <-
  read.csv("RESULTS/RankingEUSpecies.csv",
           stringsAsFactors = F)
...
# Probability of success of breeding a species in captivity
...
# Combinations for sensitivity analysis:
ns <- 20
  ...
  # Run results -------------------------------------------------------------
   ...
   for (j in 1:3) {
    ...
    for (i in 1:niter) {
      ...
      # Breeding success
      eudat$Gi <- CalcManagementExpertise(zoo.eu = eudat$zoo.eu,
      ...
      out <- apply(DecisionDf[, 1:4], 2, mean, na.rm = T)
      if (j == 1) {
        Gbenmat[i, ] <- out
      } else if (j == 2){
        Vbenmat[i, ] <- out
