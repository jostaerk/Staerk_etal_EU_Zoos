...
Iucn <-
  read.csv(
    "4_alldataclean/IucnEUTetrapods_GBIF_checked.csv",
    na.strings = c("", " ", "NA"),
    stringsAsFactors = F,
    sep = ","
  )

# Combine EU27 and European assessment categories
# When available we used the EU27 RL Categories, else we used the Euorpean assessment
Iucn$RLcatEUFinal <- Iucn$RL.catEU27
for (i in 1:nrow(Iucn)) {
  if ((Iucn$RL.catEU27[i] == "NE") | (is.na(Iucn$RL.catEU27[i]))) {
    Iucn$RLcatEUFinal[i] <- Iucn$RLcat.europe[i]
  }
}

# We exclude hybrid species
hybrids <- c("Pelophylax esculentus",
             "Pelophylax grafi",
             "Pelophylax hispanicus")

Iucn <- Iucn[which(!(Iucn$db.species %in% hybrids)), ]

# Save cleaned data
write.csv(Iucn, "4_alldataclean/IucnEUTetrapods_GBIF_checked_cleaned.csv", row.names = F)
